                    ...
                    <div class="col-md-6">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control @error('no_polisi') is-invalid @enderror"
                                   id="no_polisi" name="no_polisi" value="{{ old('no_polisi', $armada->no_polisi) }}"
                                   required>
                            @error('no_polisi')
                            ...
                            <select class="form-control @error('sopir_id') is-invalid @enderror"
                                ...
                                @foreach($sopirs as $sopir)
                                    <option value="{{ $sopir->id }}" 
                                        {{ old('sopir_id', $armada->sopir_id) == $sopir->id ? 'selected' : '' }}>
                                        {{ $sopir->nama }} - {{ $sopir->email }}
                                    </option>
                                @endforeach
                            </select>
                            <small class="form-text text-muted"><i class="fas fa-user"></i> Hanya menampilkan user dengan role Sopir</small>
                            @error('sopir_id')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                            ...
                            <select class="form-control @error('parkir_id') is-invalid @enderror"
                                ...
                                @foreach($parkirs as $parkir)
                                    <option value="{{ $parkir->id }}" 
                                        {{ old('parkir_id', $armada->parkir_id) == $parkir->id ? 'selected' : '' }}>
                                        {{ $parkir->nama }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control @error('merek') is-invalid @enderror"
                                   id="merek" name="merek" value="{{ old('merek', $armada->merek) }}">
                            @error('merek')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="form-group">
                            ...
                            <input type="number" class="form-control @error('kapasitas') is-invalid @enderror"
                                   id="kapasitas" name="kapasitas" value="{{ old('kapasitas', $armada->kapasitas) }}"
                                   min="0" step="0.1">
                            @error('kapasitas')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="form-group">
                            ...
                            <option value="tidak_tersedia" {{ old('status', $armada->status) == 'tidak_tersedia' ? 'selected' : '' }}>Tidak Tersedia (Sedang Dipesan)</option>
                            <option value="perawatan" {{ old('status', $armada->status) == 'perawatan' ? 'selected' : '' }}>Sedang Perawatan (Maintenance)</option>
                            </select>
                            @error('status')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="form-group">
                            ...
                            <textarea class="form-control @error('deskripsi') is-invalid @enderror"
                                      id="deskripsi" name="deskripsi" rows="3">{{ old('deskripsi', $armada->deskripsi) }}</textarea>
                            @error('deskripsi')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <!-- Preview Image -->
                            <div id="preview-container" class="mt-3" style="{{ $armada->gambar ? '' : 'display: none;' }}">
                                <img id="preview-image" src="{{ $armada->gambar ?? '' }}" alt="Preview" class="img-thumbnail" style="max-width: 300px;">
                            </div>
                        </div>
                    </div>
                </div>
